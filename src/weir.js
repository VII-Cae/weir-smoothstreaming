 ...
 *     get 120 small even steps instead of 8 lurches.
 *
 *   · Only the paragraph that is still growing is touched. A paragraph sealed
 *     by a blank line is never written to again.
 *
 *   · With `fade: true`, each frame's new characters are wrapped in a short-
 *     lived <span> that CSS fades in. One span per frame, not per character —
 *     at 120fps that is a couple of characters at a time, which reads as a
 *     continuous surfacing. Sealed paragraphs still never get touched, so the
 *     performance work above is preserved.
 *
 *   · When the stream ends, those spans are flattened back into plain text
 *     (see _solidify). This matters if you cache the finished innerHTML
 *     anywhere: re-inserting markup that still carries the animation class
 *     replays the whole animation at once, which reads as a twitch.
 *
 * Pair it with CSS along these lines:
 *
 *     @keyframes weir-wet { from { opacity: 0 } to { opacity: 1 } }
 *     .weir-wet { animation: weir-wet 300ms ease-out both; }
 *
 *     @media (prefers-reduced-motion: reduce) {
 *       .weir-wet { animation: none; }
 *     }
 *
 * Only opacity — translating the span makes the whole line jitter as it
 * reflows. `both` pins the starting opacity so there is no one-frame flash
...
(function (root) {
  ...
  const FADE_MAX = 48;         // past this many characters in one frame, skip the fade:
                               // a big block lighting up at once reads as a flash, not a fade
  ...
  class Flow {
     ...
     * @param opts.fade        wrap each frame's new characters for a CSS fade-in
     * @param opts.fadeClass   class on that wrapper (default "weir-wet")
    ...
    constructor(host, opts) {
      ...
      // Fading only makes sense in paragraph mode: without paragraphs the whole
      // message is one text node, so there is nothing for _solidify to flatten into.
      this.fade = !!opts.fade && !!this.block;
      this.fadeClass = opts.fadeClass || "weir-wet";
      ...
      this._tail = null;     // text node being appended to (null in fade mode: each frame starts a span)
      this._wet = false;     // this flow emitted fade spans, so settling must flatten them
    ...
    _catchUp() {
      ...
      if (this.shown < this.text.length) {
        this._write(this.text.slice(this.shown), false);   // nobody watching, no fade
    ...
    flush() {
      if (this.dead) return;
      if (this.shown < this.text.length) {
        this._write(this.text.slice(this.shown), false);
        this.shown = this.text.length;
      }
      this.ended = true;
      this._settle();
    ...
    _frame(ts) {
      ...
      if (n > 0) {
        this._write(this.text.substr(this.shown, n), this.fade && !jump && n <= FADE_MAX);
        this.shown += n;
        if (this.onGrow) this.onGrow();
      }
    ...
    _write(s, fade) {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (parts[i]) this._emit(parts[i], fade);
      }
    }

    /** Land a handful of characters.
     *  Fading: a fresh span per frame, whose CSS animation runs once and stops.
     *  Otherwise: append to the text node already there — zero new nodes, the
     *  cheapest write the DOM offers. */
    _emit(s, fade) {
      const box = this._blockNode();
      if (fade) {
        const w = this.doc.createElement("span");
        w.className = this.fadeClass;
        const t = this.doc.createTextNode("");
        t.appendData(s);
        w.appendChild(t);
        box.appendChild(w);
        this._tail = null;        // the next handful can't join this span; it has its own animation
        this._wet = true;
        return;
      }
      if (!this._tail) {
    ...
    /** Flatten the fade spans back into plain text nodes once the stream is over.
     *  Two reasons: (1) if the finished innerHTML gets cached and re-inserted
     *  later, spans still carrying the animation class replay it all at once,
     *  which reads as a twitch; (2) the DOM returns to its simplest form.
     *  Costs one paragraph-level reflow — the stream has ended, nobody sees it. */
    _solidify() {
      if (!this._wet || this.dead) return;
      this._wet = false;
      const blocks = Array.prototype.filter.call(this.host.childNodes, (c) => c.tagName);
